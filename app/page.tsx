...
const roles: { id: Role; label: string; description: string }[] = [
  ...
  { id: 'owner', label: 'مالك', description: 'يدير المساحات' },
...
export default function Home() {
  ...
  const [message, setMessage] = useState('ابحث عن مساحة مناسبة، افتح التفاصيل، ثم أرسل طلب حجز تجريبي. لوحة المالك/الأدمن منفصلة الآن.')
  ...
  const startAuth = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!saudiPhonePattern.test(phone)) return setAuthError('رقم الجوال يجب أن يكون بصيغة +9665XXXXXXXX')
    if (role === 'ksu_admin' && !ksuEmailPattern.test(email)) return setAuthError('KSU Admin يتطلب بريد @ksu.edu.sa')
    setAuthStep('otp'); setMessage(`OTP تجريبي جاهز للدور ${roleLabel}. استخدم ${MOCK_OTP}.`)
  }
  const verifyOtp = () => {
    if (otp !== MOCK_OTP) return setAuthError(`OTP غير صحيح. كود التطوير ${MOCK_OTP}`)
    setAuthError(''); setAuthStep('signed_in'); setMessage(`تم الدخول كـ ${roleLabel}. يمكنك الآن تجربة صلاحيات الدور.`)
  }
  const resetAuth = () => { setAuthStep('phone'); setOtp(''); setAuthError(''); setMessage('تم تسجيل الخروج من جلسة التطوير.') }
  ...
  const createBooking = () => {
    ...
    if (!signedIn) { setMessage('سجّل دخول mock OTP قبل طلب الحجز.'); document.getElementById('auth')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); return }
  ...
  return (
    <main className="min-h-screen w-full max-w-full overflow-x-hidden font-arabic text-wayyak-deep">
      ...
      {viewMode === 'workspace' ? <section id="auth" className="mx-auto w-full max-w-7xl overflow-hidden px-4 py-8 sm:px-6 lg:px-8"><div className="grid gap-5 rounded-[2rem] bg-white p-5 shadow-card md:grid-cols-[.9fr_1.1fr] md:p-7"><div><p className="font-english text-xs font-black uppercase tracking-[0.24em] text-wayyak-gold" dir="ltr">Minimum auth foundation</p><h2 className="mt-2 text-3xl font-black">Mock OTP للأدوار</h2><p className="mt-3 leading-8 text-wayyak-deep/65">كود التطوير <bdi dir="ltr" className="font-english font-black text-wayyak-green">123456</bdi>. الهاتف يجب أن يطابق +9665XXXXXXXX و KSU Admin يتطلب @ksu.edu.sa.</p></div><form onSubmit={startAuth} className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">الدور<select value={role} disabled={signedIn} onChange={(event) => setRole(event.target.value as Role)} className="mt-2 w-full rounded-2xl border border-wayyak-green/10 bg-wayyak-sand p-3">{roles.map((item) => <option key={item.id} value={item.id}>{item.label} — {item.description}</option>)}</select></label><label className="text-sm font-bold">رقم الجوال<input value={phone} disabled={signedIn} onChange={(event) => setPhone(event.target.value)} className="mt-2 w-full rounded-2xl border border-wayyak-green/10 bg-wayyak-sand p-3 font-english" dir="ltr" /></label><label className="text-sm font-bold sm:col-span-2">KSU Email<input value={email} disabled={signedIn} onChange={(event) => setEmail(event.target.value)} className="mt-2 w-full rounded-2xl border border-wayyak-green/10 bg-wayyak-sand p-3 font-english" dir="ltr" /></label>{authStep === 'phone' ? <button className="rounded-2xl bg-wayyak-green px-4 py-3 font-black text-white sm:col-span-2">إرسال OTP تجريبي</button> : null}{authStep === 'otp' ? <div className="grid gap-3 sm:col-span-2 sm:grid-cols-[1fr_auto]"><input value={otp} onChange={(event) => setOtp(event.target.value)} placeholder="123456" className="rounded-2xl border border-wayyak-green/10 bg-wayyak-sand p-3 text-center font-english" dir="ltr" /><button type="button" onClick={verifyOtp} className="rounded-2xl bg-wayyak-gold px-6 py-3 font-black text-wayyak-deep">تحقق ودخول</button></div> : null}{signedIn ? <button type="button" onClick={resetAuth} className="rounded-2xl bg-wayyak-deep px-4 py-3 font-black text-white sm:col-span-2">تسجيل خروج / تغيير الدور</button> : null}{authError ? <p className="rounded-2xl bg-red-50 p-3 text-sm font-bold text-red-700 sm:col-span-2">{authError}</p> : null}<p className="rounded-2xl bg-wayyak-mint p-3 text-sm font-bold text-wayyak-green sm:col-span-2">الحالة: {signedIn ? `داخل كـ ${roleLabel}` : authStep === 'otp' ? 'OTP مرسل تجريبياً' : 'بانتظار OTP'}</p></form></div></section> : null}
